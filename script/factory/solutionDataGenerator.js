...
(function(){
		NETWORK.Solution = function (networkObj) {
			...
			var busCount = this.networkObj.busDO.dOL.length;
			for(var indexBus = 0; indexBus < busCount; indexBus++) {
				this.addNodeSolutionData(this.networkObj.busDO.dOL[indexBus]);
			}
			
			var branchCount = this.networkObj.branchDO.dOL.length;
			for(var branchIndex = 0; branchIndex < branchCount; branchIndex++) {
				this.addEdgeSolutionData(this.networkObj.branchDO.dOL[branchIndex]);
			}
		};
	...
	* Multiplies all the values by MVA base so as to present the data in the correct format.*/
	NETWORK.Solution.prototype.addEdgeSolutionData = function(edgeObj) {
		...
		var source = this.networkObj.busDO.dOL[nodeIndexMap[edgeData.fbus]];
		var target =  this.networkObj.busDO.dOL[nodeIndexMap[edgeData.tbus]];
		var solData = {};
		...
		solData["p-s-t"] = ((a1 - a2 - a3) * parseFloat(NET_OBJ.BaseMVA)).toFixed(4);
		...
		solData["q-s-t"] = ((-b1 + b2 - b3) * parseFloat(NET_OBJ.BaseMVA)).toFixed(4);
		...
		solData["p-t-s"] = ((c1 - c2 - c3) * parseFloat(NET_OBJ.BaseMVA)).toFixed(4);
		...
		solData["q-t-s"] =  ((-d1 + d2 - d3) * parseFloat(NET_OBJ.BaseMVA)).toFixed(4);
